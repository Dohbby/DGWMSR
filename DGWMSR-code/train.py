...
# parser.add_argument('--train_file', type=str, default="/home/data/train/test-64.h5")  # 训练 h5文件目录
parser.add_argument('--train_file', type=str, default="/home/data/dataset3090/train-h5/train-4x-64-48.h5")  # 训练 h5文件目录
...
parser.add_argument("--step_size", type=int, default=[15,30,40,45], help="learning rate decay per N epochs")
...
parser.add_argument("--output_dir", type=str, default="./mixnumber/")
...
parser.add_argument("--validtext", type=str, default="./mixnumber/Validation.txt", help='text file to store validation results')
...
trainset=H5Dataset(args.train_file)
training_data_loader = DataLoader(dataset=trainset, num_workers=args.threads * ngpu(gpus), batch_size=args.batch_size, shuffle=True, drop_last=True)
...
class PerceptualLoss(nn.Module):
    def __init__(self, feature_layers=[0, 5, 10, 19], use_gpu=True):
        ...
        # 选择用于感知损失的特征层（这里选择了第 0, 5, 10, 19 层）
        self.feature_layers = feature_layers
        self.vgg = vgg
        self.criterion = nn.MSELoss()  # 通常用 MSE 计算特征差异
    ...
    def forward(self, pred, target):
        # 确保输入是 RGB 格式，范围在 [0, 1]，且形状为 (B, C, H, W)
        if pred.dim() == 3:  # 如果输入是 (C, H, W)，增加 batch 维度
            pred = pred.unsqueeze(0)
            target = target.unsqueeze(0)

        # VGG 期望输入为 3 通道（RGB），如果你的图像是单通道，需要调整
        if pred.size(1) == 1:
            pred = pred.repeat(1, 3, 1, 1)  # 将单通道复制为 3 通道
        ...
        return loss / len(self.feature_layers)  # 平均多层损失
    ...
    def _get_features(self, x):
        ...
        for i, layer in enumerate(self.vgg):
            ...
            if i in self.feature_layers:
                features.append(x)
            if i == max(self.feature_layers):  # 提前终止
                break
        return features

l1_criterion = nn.L1Loss()
perceptual_criterion = PerceptualLoss(feature_layers=[0, 5, 10, 19], use_gpu=True)

# 超参数：感知损失的权重
lambda_perceptual = 0.1  # 调整这个权重来平衡 L1 和感知损失
...
def train(epoch):
    ...
    with tqdm(training_data_loader, unit="batch") as tepoch:
        ...
        for iteration, batch in enumerate(tepoch, 1):
            lr, left, right, gt= batch[0], batch[1], batch[2], batch[3]
            ...
            #
            # flops, params = profile(model, inputs=(lr_tensor))
            # flops, params = clever_format([flops, params], "%.3f")  # 格式化输出
            #
            # print(f"Params: {params}")
            # print(f"FLOPs: {flops}")

            # sr_tensor = model(left,right,lr_tensor)
            ...
            print("Epoch: {} Iteration[{}/{}]  Learning rate: {:.5f}  Loss: {:.4f} PSNR:{}".format(epoch, iteration,
                                                                                                   len(training_data_loader),
                                                                                                   scheduler.optimizer.param_groups[
                                                                                                       0]['lr'],
                                                                                                   loss_meter.avg * accum_iter,
                                                                                                   psnr),file=txt_write)
...
def print_network(model):
    ...
    macs, params = get_model_complexity_info(model, (1, 64, 64), as_strings=True, print_per_layer_stat=True, verbose=True)
    # 计算 FLOPs（假设 1 MAC = 2 FLOPs）
    if 'GMac' in macs:
