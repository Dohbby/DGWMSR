...
parser.add_argument("--psnr1text", type=str, default="/home/CFAT-yuanma/mixnumber/PSNR-each.txt", help='text file to store validation results')
parser.add_argument("--psnr2text", type=str, default="/home/CFAT-yuanma/mixnumber/PSNR.txt", help='text file to store validation results')
...
# model=srcnn.SRCNN()
# from model import  camixsr
# model=camixsr.CAMixerSR()
###################---------Loss_Function---------###################
...
for epoch in range(31,32):
    checkpoint_file =  r"/home/CFAT-yuanma/pth/srformer-checkpoint_31.pth"
    ...
    with torch.no_grad():
        ...
        for mat_num in range(1,2):
            ...
            for data in test_dataloader:
                ...
                label = torch.cat([label,torch.flip(label, [2])], 2)[:, :, :h_old + h_pad, :]
                label = torch.cat([label, torch.flip(label, [3])], 3)[:, :, :, :w_old + w_pad]  # [1, 1, 192, 320]

                # imgs=torch.cat((lr,left,right),dim=1)
                # print('111111',imgs.shape)
                # measure_inference_time(model, lr)

                # output = model(label,right,left) # [1, 1, 192, 320]
                output = model(label) # [1, 1, 192, 320]
            ...
            savemat('/home/CFAT-yuanma/mixnumber/srformer.mat'.format(epoch, mat_num), new)
